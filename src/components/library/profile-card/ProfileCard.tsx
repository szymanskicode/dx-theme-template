...
export const ProfileCard = ({ items = [], colCount = 2, title = "", cardData, onDataChanged, children, wrapperCssClass }: React.PropsWithChildren<ProfileCardProps>) => {
            ...
            {items.map((item, index) => (
              ...
              >
                ...
                {item.dataField === "status" && (
                  <StatusSelectBox labelMode="hidden" stylingMode="filled" value={cardData[item.dataField]} onValueChange={onFieldChange(item.dataField)} />
                )}
                {item.dataField === "supervisor" && (
                  <PicturedItemSelectBox label={item.label} value={cardData[item.dataField]} items={item.itemsList} onValueChange={onFieldChange(item.dataField)} />
                )}
              </Item>
            ))}
          </Form>
