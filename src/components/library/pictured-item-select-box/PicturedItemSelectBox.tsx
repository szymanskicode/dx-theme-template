...
const fieldRender = (data: any) => {
  return (
    <div className="pictured-item-select-field">
      <img alt={data.name} className="pictured-item-image" src={`data:image/png;base64,${data.image}`} />
      <TextBox hoverStateEnabled={false} inputAttr={{ picturedItemEditorInput: "" }} readOnly value={data.name} />
    </div>
  );
};
...
const ItemRender = (data: any) => {
  return (
    <>
      <img alt={data.name} className="pictured-item-image" height="20px" src={`data:image/png;base64,${data.image}`} />
      {data.name}
    </>
  );
};
